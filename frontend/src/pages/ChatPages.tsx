...
function ThinkingPanel({ compact = false, product = false, recommend = false }: { compact?: boolean; product?: boolean; recommend?: boolean }) {
  ...
  return <div className={`${compact ? 'py-1' : 'mt-5'} flex items-center gap-3`} aria-live="polite"><AiMotion size={compact ? 'tiny' : 'loading'}/><div><p className="text-[12px] font-medium text-[#666]">{text}</p><div className="mt-2 w-fit rounded-full bg-[#5b5b5b] px-3 py-2"><TypingDots/></div></div></div>
}
...
function UserMessage({ text, createdAt, pending = false }: { text: string; createdAt?: string; pending?: boolean }) {
  return <div className="flex flex-col items-end"><div className="w-fit max-w-[84%] rounded-[20px] border border-[#cfe0ff] bg-white px-4 py-2.5 text-[14px] leading-6 text-black"><MessageContent text={text}/></div><span className="mt-1.5 px-1 text-[9px] text-[#a0a5ac]">{pending ? '보내는 중…' : createdAt ? messageTime(createdAt) : ''}</span></div>
}
...
function AssistantMessage({ message, recommend, onEvidence }: { message: Conversation['messages'][number]; recommend: boolean; onEvidence: () => void }) {
  ...
  return <article className="w-full"><div className="w-fit max-w-[82%] rounded-[20px] bg-black px-4 py-3.5 text-[14px] leading-6 text-white"><MessageContent text={message.content} markdown inverse/>{message.status === 'FALLBACK' && <div className="mt-3 flex items-start gap-2 border-t border-white/15 pt-3 text-[10px] leading-4 text-white/60"><ShieldCheck size={13} className="mt-0.5 shrink-0"/>외부 AI 연결 없이 저장된 내 데이터로 답했어요.</div>}</div><span className="mt-1.5 block px-1 text-[9px] text-[#a0a5ac]">{messageTime(message.createdAt)}</span>{recommend && <RecommendedProductLinks refs={message.evidenceRefs}/>} {hasEvidence && <EvidenceSummary refs={message.evidenceRefs} webSources={message.webSources || []} onOpen={onEvidence}/>}</article>
}

function RetryCard({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <div className="mt-5 rounded-[18px] border border-[#f1d8d8] bg-[#fff7f7] p-4 text-xs leading-5 text-[#9a4848]"><p>{message}</p><button type="button" onClick={onRetry} className="mt-3 inline-flex items-center gap-1.5 font-semibold"><RefreshCw size={14}/>같은 메시지 다시 보내기</button></div>
}
...
function MessageContent({ text, markdown = false, inverse = false }: { text: string; markdown?: boolean; inverse?: boolean }) {
  ...
  return <div className={inverse ? 'min-w-0 text-[14px] leading-6 text-white' : 'min-w-0 text-[14px] leading-6 text-ink'}>
    <ReactMarkdown
      skipHtml
      ...
      components={{
        ...
        strong: ({ children }) => <strong className={inverse ? 'font-bold text-white' : 'font-bold text-ink'}>{children}</strong>,
