...
export function ExperiencePage() {
  ...
  const experience = useQuery({ queryKey: ['experience', experienceId], queryFn: () => api.experience(experienceId) })
  ...
  return <Screen nav={false} className="pb-32">
    ...
    <div className="px-5 py-6">
      ...
      <button onClick={() => navigate(`/experiences/${experienceId}/record?end=1`)} className="mx-auto mt-8 block text-xs font-semibold text-muted underline decoration-line underline-offset-4">느낌을 남기고 일찍 마치기</button>
    </div>
...
export function RecordPage() {
  ...
  const record = useMutation({ mutationFn: async () => {
    const value = await api.recordExperience(experienceId, { sentiment, note, tags, discomfort })
  ...
  if (saved) return <SavedRecordResult saved={saved} reviewCompleted={experience.data.reviewDue} experienceEnded={endAfterSave} onRescue={openRescueChat} onContinue={() => navigate('/')} onComplete={() => complete.mutate()}/>
  const options = [
  ...
  return <Screen nav={false} className="pb-28">
    ...
    <div className="px-5 py-6"><p className="text-xs font-bold text-accent">{experience.data.title}</p><h1 className="mt-2 text-[25px] font-bold leading-8 tracking-[-.04em]">지금까지 써보니<br/>어떠셨나요?</h1>
      <div className="mt-6 space-y-2">{options.map(option => <button key={option.value} onClick={() => setSentiment(option.value)} className={`flex w-full items-center gap-4 rounded-[20px] border p-4 text-left transition ${sentiment === option.value ? 'border-accent bg-accent-soft' : 'border-line bg-white'}`}><span className="grid size-10 place-items-center rounded-full bg-white text-xl">{option.emoji}</span><span className="flex-1"><b className="block text-sm">{option.title}</b><span className="mt-1 block text-xs text-muted">{option.body}</span></span>{sentiment === option.value && <Check size={18} className="text-accent"/>}</button>)}</div>

      <section className="mt-7"><h2 className="text-sm font-bold">어떤 느낌이었나요? <span className="font-normal text-muted">선택</span></h2><div className="mt-3 flex flex-wrap gap-2">{tagOptions.map(tag => <button key={tag} onClick={() => setTags(value => value.includes(tag) ? value.filter(x => x !== tag) : [...value, tag])} className={`rounded-full border px-3 py-2 text-xs font-semibold ${tags.includes(tag) ? 'border-ink bg-ink text-white' : 'border-line bg-white text-muted'}`}>{tag}</button>)}</div></section>
      ...
      {record.error && <p className="mt-4 text-sm text-danger">{record.error.message}</p>}
    </div>
...
function SavedRecordResult({ saved, reviewCompleted, experienceEnded, onRescue, onContinue, onComplete }: { saved: SavedRecord; reviewCompleted: boolean; experienceEnded: boolean; onRescue: () => void; onContinue: () => void; onComplete: () => void }) {
  ...
  return <Screen nav={false}><TopBar title={reviewCompleted ? '7일 경험 완료' : experienceEnded ? '사용 경험 완료' : '기록 완료'}/><div className="px-5 py-10 text-center"><div className="mx-auto grid size-16 place-items-center rounded-full bg-lime text-ink"><Check size={28}/></div><h1 className="mt-6 text-[26px] font-bold tracking-[-.04em]">{reviewCompleted ? '7일의 경험이 쌓였어요' : experienceEnded ? '지금까지의 경험을 남겼어요' : '오늘의 경험을 남겼어요'}</h1><p className="mx-auto mt-3 max-w-72 text-sm leading-6 text-muted">이 기록은 다음 제품을 살펴보거나 과거 경험을 비교할 때 다시 쓰입니다.</p>
    ...
    {!completed && <button onClick={onComplete} className="mt-5 text-xs font-semibold text-muted underline decoration-line underline-offset-4">이 경험은 여기서 마치기</button>}
  </div></Screen>
}
...
export function RoutineEditPage() {
  ...
  useEffect(() => { if (!initialized && current.data) {
  ...
  const save = useMutation({ mutationFn: () => api.replaceRoutine('내 스킨케어 루틴', routineItems()), onSuccess: value => { queryClient.invalidateQueries(); navigate(`/experiences/${value.id}`) } })
  const finishAndSave = async (sentiment?: 'LIKED' | 'UNSURE' | 'DISAPPOINTED') => {
    ...
    try {
      if (sentiment) await api.recordExperience(active.id, { sentiment, note: '', tags: [], discomfort: 'UNKNOWN' })
  ...
  if (products.isPending) return <Screen nav={false}><TopBar title="루틴 편집" back/><Loading/></Screen>
