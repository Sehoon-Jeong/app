...
| `패트리 접시 애니메이션.mp4` | `패트리 접시(투명).png`의 움직이는 버전. | MP4(~11MB) | 흰/투명 | 페트리 모티프를 모션으로 강조할 때. |
| `로딩창 애니메이션.mp4` | 맑은 유리 버블/방울이 생성·유동하는 모습. 정지본 없음. | MP4(~9MB) | 흰/투명 | **로딩 인디케이터.** 데이터·AI 응답 대기 화면. |
