...
이 문서는 3주 MVP에서 반드시 성립해야 하는 제품 계약을 정의한다. 화면 배치, API 형식과 테이블 필드는 각각 기능명세, OpenAPI와 데이터 모델에서 정한다.
...
| 실험 기간 | 기본 14일이며 사용자가 7~28일 안에서 바꿀 수 있다. | 일정 운영을 위한 기본값일 뿐 안전성·효능을 판단하는 임상 기준이 아니다. |
...
| ID | 우선순위 | 요구사항 | 확인 기준 |
| --- | --- | --- | --- |
| FR-AUTH-001 | P0 | 사용자는 이메일 인증으로 로그인·로그아웃할 수 있다. | 인증되지 않은 사용자는 개인 루틴과 실험 API에 접근할 수 없다. |
...
| FR-EXP-003 | P0 | 실험 계획에는 적용할 시간대와 순서, 시작일·종료일, 관찰 시점과 항목이 포함된다. | 계획을 저장하면 해당 관찰 퀘스트가 생성된다. |
...
| FR-ARC-003 | P0 | 사용자는 완료·취소·보류한 실험을 Beauty Archive에서 시간순으로 확인할 수 있다. | 목록과 상세에서 당시 안정 루틴, 제품, 기간과 결과를 본다. |
...
| FR-LAB-002 | P0 | 필수 결과까지 기록한 실험 하나가 연구실에 하나의 완료 연구 기록으로 추가된다. | 같은 실험을 수정하거나 다시 열어도 기록이 중복 생성되지 않는다. |
...
| NFR-PRV-001 | 최소수집 | P0 | 피부 관찰 원문과 전성분 사진을 PostHog·Sentry 이벤트에 보내지 않는다. |
...
| NFR-OBS-001 | 측정 | P0 | 입력 완료, 추천 확인, 실험 시작, 필수 관찰, Rescue, 결과 저장, 다음 실험 시작 이벤트를 가명 식별자로 수집한다. |
...
2. 변경 시 기능명세, GitHub 이슈, 데이터 모델과 검증 항목을 함께 확인한다.
3. 기능명세에 연결되지 않은 P0·P1 요구사항은 구현 준비가 끝난 것으로 보지 않는다.
