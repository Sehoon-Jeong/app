...
AI는 현재 기록과 관련된 과거 평가·태그를 검색하고, 사용자가 실제로 남긴 범위 안에서 정리한다. 제품 정보, 사용자 기록, AI 해석을 구분한다. 자유 메모 원문은 사용자가 명시적으로 요청하지 않으면 AI 공급자에게 보내지 않는다.
...
- 현재 질문과 관련된 개인 기록을 연결하고, 제품·성분 외부 사실은 OpenAI API 안의 웹 검색으로 확인한다.
- 공통점·차이·반대 기록·불확실성을 설명한다.
- 필요한 추가 질문을 최대한 적게 한다.

서버는 소유권, 제품·루틴 ID, 상태 전이, 개인 근거 참조, HTTPS 출처와 출력 schema를 검증한다. 외부 사실은 문장별 인용을 보여주며 AI 원문 출력은 사용자 사실이나 최종 상태가 아니다.
