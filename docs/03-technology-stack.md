...
AI는 전성분 사진과 자연어 경험을 구조화하고, 서버가 계산한 결과를 사용자가 이해하기 쉽게 설명한다. 제품 추천에서는 서버가 카탈로그 후보와 비교 사실을 제한하고 AI가 그 안에서 우선순위와 설명을 제안한다. 서버는 제품 ID와 출력 구조를 검증하며, 실패하면 일반 규칙 기반 순서로 대체한다.
