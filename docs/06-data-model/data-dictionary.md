...
테이블 사이의 관계를 먼저 보려면 [ERD와 데이터 모델](./README.md)을 읽고, 테이블·필드·제약의 정확한 의미는 이 문서에서 확인한다.

## 목차

| 영역 | 포함 내용 |
| --- | --- |
| [표기와 공통 규칙](#표기) | ID, 공통 시각·버전 필드, FK와 JSONB 원칙 |
...
- 열거형, CHECK와 복합 FK의 실제 문법은 Flyway에서 정하되 이 문서의 허용값과 불변식을 바꾸지 않는다.
